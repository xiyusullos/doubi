...
Set_config_user(){
	...
	stty erase '^H' && read -p "(默认: xiyusullos):" ssr_user
	[[ -z "${ssr_user}" ]] && ssr_user="xiyusullos"
...
Set_config_port(){
	...
	stty erase '^H' && read -p "(默认: 20000):" ssr_port
	[[ -z "$ssr_port" ]] && ssr_port="20000"
...
Set_config_password(){
	echo "请输入要设置的用户 密码"
	stty erase '^H' && read -p "(默认: xiyusullos):" ssr_password
	[[ -z "${ssr_password}" ]] && ssr_password="xiyusullos"
